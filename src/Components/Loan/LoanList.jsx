...
export const LoanList = (props) => {
  ...
  const [reminderInstallationDate, setReminderInstallationDate] =
    useState(false);
  ...
  const fetchReminderToday = async (pageNumber = 1) => {
    await axios
      .get(`/api/loans/reminder?page=${pageNumber}`, {
        params: { search: searchKey, filter: filters },
      })
      .then(({ data }) => {
  ...
  const fetchLoansTodayFollowUp = (pageNumber = 1) => {
    axios
      .get(`/api/loans/todayfollowup?page=${pageNumber}`, {
        params: { search: searchKey, filter: filters },
      })
      .then(({ data }) => {
  ...
  const fetchNotContactToday = (pageNumber = 1) => {
    axios
      .get(`/api/loans/notContacted?page=${pageNumber}`, {
        params: { search: searchKey, filter: filters },
      })
      .then(({ data }) => {
  ...
  const handleFilter = (e, pageNumber = 1) => {
    axios
      .get(activeTab + `?page=${pageNumber}`, { params: { filter: e } })
      .then(({ data }) => {
        if (activeTab === "/api/loan") {
          setLoans(data);
        } else if (activeTab === "/api/loans/todayfollowup") {
          ...
          <Tabs className="cl-tabs">
              ...
              <table className="table">
                ...
                <tbody>
                  {loans?.data?.map((item, index) => {
                    ...
                    var lastitem =
                      item.loan_details[item.loan_details.length - 1];
                            ...
                            <Dropdown className="table-dropdown">
                              ...
                              <Dropdown.Menu>
                                ...
                                <Dropdown.Item>
                                  <div>
                                    <Link
                                      to={`/customer-loandetail-payment/${item.id}`}
            ...
            <TabPanel className="today tabdetail" id="tabtoday">
              <table className="table">
                ...
                <tbody>
                  {todayfollowup?.data?.map((item, index) => {
                    ...
                    var lastitem =
                      item.loan.loan_details[item.loan.loan_details.length - 1];
                    var pay_date = lastitem ? lastitem.created_at : "";
                    {
                        ...
                        <td>
                          <Zoom>
                            <img
                              ...
                              src={
                                window.baseurl +
                                `/storage/${item.loan.customer.image}`
                              }
                            />
                          </Zoom>
                        </td>
                            ...
                            <Dropdown className="table-dropdown">
                              ...
                              <Dropdown.Menu>
                                ...
                                <Dropdown.Item>
                                  <div>
                                    <Link
                                      to={`/customer-loandetail-payment/${item.loan.id}`}
                                      className="fa fa-money"
                                    >
                                      Pay
                                    </Link>
              ...
              <div>
                <Pagination
                  activePage={
                    todayfollowup?.current_page
                      ? todayfollowup?.current_page
                      : 0
                  }
                  itemsCountPerPage={
                    todayfollowup?.per_page ? todayfollowup?.per_page : 0
                  }
                  totalItemsCount={
                    todayfollowup?.total ? todayfollowup?.total : 0
                  }
                  onChange={(pageNumber) => {
            ...
            <TabPanel className="notcontacted tabdetail" id="tabnotconnected">
              ...
              <div>
                <Pagination
                  activePage={
                    notcontactedtoday?.current_page
                      ? notcontactedtoday?.current_page
                      : 0
                  }
                  itemsCountPerPage={
                    notcontactedtoday?.per_page
                      ? notcontactedtoday?.per_page
                      : 0
                  }
                  totalItemsCount={
                    notcontactedtoday?.total ? notcontactedtoday?.total : 0
                  }
                  onChange={(pageNumber) => {
                    fetchLoansTodayFollowUp(pageNumber);
                  }}
                                ...
                                <Dropdown.Item>
                                  <div>
                                    <a
                                      ...
                                      onClick={() => deleteLoan(item.loan.id)}
                                    >
                                      Delete
                                    </a>
                                  </div>
